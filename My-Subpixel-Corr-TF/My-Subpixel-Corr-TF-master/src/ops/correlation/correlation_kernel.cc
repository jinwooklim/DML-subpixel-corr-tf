...
class CorrelationKernel : public OpKernel {
  public:
    ...
    void Compute(OpKernelContext *ctx) override {
      ...
      float neighborhood_grid_radius = float(max_displacement) / stride_2;
      float neighborhood_grid_width  = neighborhood_grid_radius * 2.0 + 1.0;
      //std::cout << "\n";
      //std::cout << "max_displacement : " << max_displacement << "\n";
      //std::cout << "neighborhood_grid_radius : " << neighborhood_grid_radius << "\n";
      //std::cout << "neighborhood_grid_width : " << neighborhood_grid_width << "\n";

      // Original
      int output_channels          = neighborhood_grid_width * neighborhood_grid_width; // s > 1.0
      //nt output_channels          = int((neighborhood_grid_width-1.0) / stride_2 + 1.0) * ((neighborhood_grid_width-1.0) / stride_2 + 1.0); // jy // s < 1.0
      //int output_channels          = int(neighborhood_grid_radius+1) * int(neighborhood_grid_radius+1);
      //int output_channels = ;
