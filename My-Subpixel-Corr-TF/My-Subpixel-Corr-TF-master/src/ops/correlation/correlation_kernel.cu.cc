...
//
//extern __shared__ char offset_list_char[];
//int *offset_list = (int *)offset_list_char;
//
  
  // New kernel_size
  //int new_kernel_size = 2*kernel_size-1;
...
//
//int offset_idx = 0;
//for (int i = 0; i < neighborhood_grid_width; i++) {
//	for (int j = 0; j < neighborhood_grid_width; j++) {
//		offset_list[offset_idx] = i;
//		offset_list[offset_idx+1] = j;
//		offset_idx += 2;
//	}
//}
//
...
// Version 2 -> Continue ...
  // Compute correlation
  //for (int out_channel = 0; out_channel < (neighborhood_grid_width*2-1) * (neighborhood_grid_width*2-1); out_channel++) { // jy
  //for (int out_channel = 0; out_channel < ((neighborhood_grid_width-1) / stride_2 + 1) * ((neighborhood_grid_width-1) / stride_2 + 1); out_channel++) {
  for (int out_channel = 0; out_channel < out_channels; out_channel++) {
    sum[ch_off] = 0;

    // Original
    float s2o = (float(out_channel % int(neighborhood_grid_width)) - neighborhood_grid_radius) * stride_2;
    float s2p = (float(out_channel / int(neighborhood_grid_width)) - neighborhood_grid_radius) * stride_2;
    int x2  = x1 + int(s2o);
    int y2  = y1 + int(s2p);
    
    //int s2o = ((out_channel) % (2*neighborhood_grid_width-1) - (neighborhood_grid_width+1)) * stride_2;
    //int s2p = ((out_channel) / (2*neighborhood_grid_width-1) - (neighborhood_grid_width+1)) * stride_2;
    //int x2  = x1 + s2o/2;
    //int y2  = y1 + s2p/2;
	...
	for (int ch = ch_off; ch < in_channels; ch += (WARPS_PER_BLOCK * THREADS_PER_WARP)) {
	  ...
	  if(int(2.0*s2o)%2 == 0 && int(2.0*s2p)%2 == 0){
	  	int idx2 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i) * in_channels + ch;
	  	sum[ch_off] += patch_data[idxPatchData] * input_b[idx2];
                //printf("k");
	  }

	  if(int(2.0*s2o)%2 == 1 && int(2.0*s2p)%2 == 0){
	  	int idx2 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i) * in_channels + ch;
	  	int idx2_1 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i + 1) * in_channels + ch;
	  	sum[ch_off] += patch_data[idxPatchData] * ((input_b[idx2] + input_b[idx2_1])/2);
                //printf("a");
	  }

	  if(int(2.0*s2o)%2 == 0 && int(2.0*s2p)%2 == 1){
		int idx2 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i) * in_channels + ch;
	  	int idx2_1 = ((item * in_height_padded + y2 + j + 1) * in_width_padded + x2 + i) * in_channels + ch;
	  	sum[ch_off] += patch_data[idxPatchData] * ((input_b[idx2] + input_b[idx2_1])/2);
		//printf("b");
	  }

	  if(int(2.0*s2o)%2 == 1 && int(2.0*s2p)%2 == 1){
		int idx2 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i) * in_channels + ch;
	  	int idx2_1 = ((item * in_height_padded + y2 + j + 1) * in_width_padded + x2 + i) * in_channels + ch;
		int idx2_2 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i + 1) * in_channels + ch;
		int idx2_3 = ((item * in_height_padded + y2 + j + 1) * in_width_padded + x2 + i + 1) * in_channels + ch;
	  	sum[ch_off] += patch_data[idxPatchData] * ((input_b[idx2] + input_b[idx2_1] + input_b[idx2_2] + input_b[idx2_3])/4);  
		//printf("c");
	  }
