...
class CorrelationGradKernel : public OpKernel {
  public:
    ...
    void Compute(OpKernelContext *ctx) override {
      ...
      const float neighborhood_grid_radius = (float)max_displacement / stride_2;
      const float neighborhood_grid_width  = neighborhood_grid_radius * 2.0 + 1.0;
      const int out_channels             = (int)neighborhood_grid_width * (int)neighborhood_grid_width;
