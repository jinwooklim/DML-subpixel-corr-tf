            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                with slim.arg_scope([slim.conv2d], weights_regularizer=weights_regularizer):
                    with slim.arg_scope([slim.conv2d], stride=2):
                        ...
                        # input_a, input_b, kernel_size, max_displacement, stride_1, stride_2, padding)
                        # cc = correlation(conv_a_3, conv_b_3, 1, 20, 1, 2, 20)
                        # cc_relu = LeakyReLU(cc)

                        # input_a, input_b, kernel_size, max_displacement, stride_1, stride_2, padding)
                        #cc = correlation(conv_a_3, conv_b_3, 3, 10, 1, 2, 21)
                        print("conv_a_3 : ", conv_a_3)
                        print("conv_b_3 : ", conv_b_3)
                        #cc = correlation(conv_a_3, conv_b_3, 3, 20, 1.0, 2.0, 21)
                        #cc = correlation(conv_a_3, conv_b_3, 3, 5, 1.0, 0.5, 6)
                        
                        #kernel_size = 3
                        #max_displacement = 20 #5#80 #20
                        #stride_1 = 1.0
                        #stride_2 = 2.0 #0.5 #2.0#0.5
                        #padding = 21 #6#81 #21
                        #cc = correlation(conv_a_3, conv_b_3, kernel_size, max_displacement, stride_1, stride_2, padding)
                        
                        #cc = correlation(conv_a_3, conv_b_3, 3, 20, 1.0, 2.0, 21)
                        #cc = correlation(conv_a_3, conv_b_3, 3, 5, 1.0, 0.5, 6)

                        cc = correlation(conv_a_3, conv_b_3, 3, 20, 1.0, 0.5, 21)
                        
                        print("cc : ", cc)
                        cc_relu = LeakyReLU(cc)
                        print("cc_relu : ", cc_relu)
                        #exit()
                    ...
                    conv3_1 = slim.conv2d(pad(net), 256, 3, scope='conv3_1') # Problem : input depth must be evenly divisible by filter depth
