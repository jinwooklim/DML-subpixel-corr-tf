...
class FlowNetC(Net):
            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                with slim.arg_scope([slim.conv2d], weights_regularizer=weights_regularizer):
                    with slim.arg_scope([slim.conv2d], stride=2):
                        ...
                        # conv_a_sub = slim.conv2d(pad(conv_a_3, 2), 256, 5, scope='conv3_sub')
                        ...
                        # conv_b_sub = slim.conv2d(pad(conv_b_3, 2), 256, 5, scope='conv3_sub', reuse=True)

                        # Compute cross correlation with leaky relu activation
                        # input_a, input_b, kernel_size, max_displacement, stride_1, stride_2, padding, rate
                        # cc = correlation(conv_a_3, conv_b_3, 1, 20, 1, 2, 20) # 441
                        # cc = correlation(conv_a_3, conv_b_3, 1, 10, 1, 1, 10, 1.0) #-> ok

                        # with tf.device('/gpu:1'):
                        print("\ninput_conv_a : ", conv_a_3, end="\n\n")
                        print("\ninput_conv_b : ", conv_b_3, end="\n\n")
                        # cc = correlation(conv_a_4, conv_b_4, 1, 20, 1, 1, 20, 2.0) # 6561 == 81^2 # almost OOM
                        cc = correlation(conv_a_3, conv_b_3, 1, 10, 1, 1, 10, 2.0)  # 1681
                        # cc = correlation(conv_a_3, conv_b_3, 1, 5, 1, 1, 5, 2.0) # 441
                        print("\ncc : ", cc, end="\n\n")
                        
                        cc_relu = LeakyReLU(cc)
                    cc_relu_441 = slim.conv2d(cc_relu, 441, 1, scope='cc_relu_441')
                    cc_relu_441 = LeakyReLU(cc_relu_441)
                    ...
                    print("\nnetA_conv : ", netA_conv, end="\n\n")
                    print("\ncc_relu : ", cc_relu_441, end="\n\n")
                    net = tf.concat([netA_conv, cc_relu_441], axis=3)
                    #
                    # B, H, W, C = conv_a_3.get_shape().as_list()
                    # print("\nnet : ", net, end="\n\n")
                    # net = tf.image.resize_images(net, [H, W], method=0)
                    # print("\nnet : ", net, end="\n\n")
                    #
    ...
    def loss(self, flow, predictions):
        ...
        # loss = tf.losses.compute_weighted_loss(losses, [0.16, 0.04, 0.04, 0.02, 0.01])
