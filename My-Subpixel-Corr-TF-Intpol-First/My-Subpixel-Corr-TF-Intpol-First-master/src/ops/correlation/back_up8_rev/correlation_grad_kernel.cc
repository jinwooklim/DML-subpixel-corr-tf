...
//        const int out_width  = ceil((float)(padded_width - border_size * 2) / stride_1);
        const int out_height = ceil((float)(padded_interpolated_height - border_size * 2) / stride_1);
        const int out_width  = ceil((float)(padded_interpolated_width - border_size * 2) / stride_1);
...
//        auto output_b_gradient = output_b_gradient_t->tensor<float, 4>();
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<float>::value, TensorShape({ batch_size, interpolated_height, interpolated_width, in_channels }), &interpolated_input_a_t));
        OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<float>::value, TensorShape({ batch_size, interpolated_height, interpolated_width, in_channels }), &interpolated_input_b_t));
        OP_REQUIRES_OK(ctx, ctx->allocate_temp(DataTypeToEnum<float>::value, TensorShape({ batch_size, interpolated_height, interpolated_width, in_channels }), &interpolated_gradients_t));
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_output(0, interpolated_input_a_t.shape(), &interpolated_output_a_gradient_t));
        OP_REQUIRES_OK(ctx, ctx->allocate_output(1, interpolated_input_b_t.shape(), &interpolated_output_b_gradient_t));
