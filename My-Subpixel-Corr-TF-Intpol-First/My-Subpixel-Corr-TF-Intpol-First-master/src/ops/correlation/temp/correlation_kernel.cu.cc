...
__global__ void CorrelateData(int          batch_size,
                              ...
                              int          in_channels,
                              float rate,
                              int          in_width_padded_interpolation,
                              int          in_height_padded_interpolation,                             
                              const float *input_a, 
                              const float *input_b,
                              float       *output) {
  ...
  int x1     = blockIdx.x * (int)stride_1 + max_displacement;
  int y1     = blockIdx.y * (int)stride_1 + max_displacement;
  int item   = blockIdx.z;
  ...
  for (int j = 0; j < kernel_size; j++) {
    ...
    for (int i = 0; i < kernel_size; i++) {
      ...
      for (int ch = ch_off; ch < in_channels; ch += (WARPS_PER_BLOCK * THREADS_PER_WARP)) {
        int idx1 = ((item * in_height_padded + y1 + j) * in_width_padded + x1 + i) * in_channels + ch;
        int idxPatchData = ji_off + ch;
        patch_data[idxPatchData] = input_a[idx1]; // input_a [b, w, h, c] -> patch_data
      }
    }
  }
  ...
  for (int out_channel = 0; out_channel < out_channels; out_channel++) {
    ...
    int s2o = (out_channel % neighborhood_grid_width - neighborhood_grid_radius) * (int)stride_2; // strided2 가 항상 1
    int s2p = (out_channel / neighborhood_grid_width - neighborhood_grid_radius) * (int)stride_2; // strided2 가 항상 1

    int x2 = (int)rate * x1 + s2o;
    int y2 = (int)rate * y1 + s2p;
...
//           int idx2 = ((item * in_height_padded + y2 + j) * in_width_padded + x2 + i) * in_channels + ch;
          int idx2 = ((item * in_height_padded_interpolation + y2 + j) * in_width_padded_interpolation + x2 + i) * in_channels + ch;
          int idxPatchData = ji_off + ch;
...
void Correlation(const GPUDevice& device,
  ...
  CorrelateData << < totalBlocksCorr, threadsPerBlock, shared_memory_per_block * sizeof(float),
    ...
    kernel_size, stride_1, stride_2, 
    in_width_padded, in_height_padded, 
    in_channels, rate,
    in_width_padded_interpolation, in_height_padded_interpolation,
