   ...
   const float *gradient)
{
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        int k_b = index % in_channels;                                     // channels
        int x_b = (index / in_channels) % interpolated_width + pad_size;             // w-pos
        int y_b = (index / in_channels / interpolated_width) % interpolated_height + pad_size; // h-pos

        // Version # 3
        int k_a = k_b;
        int x_a = (x_b - pad_size)/2 + pad_size;
        int y_a = (y_b - pad_size)/2 + pad_size;
        ...
        int xmin = (x_a - 2 * kernel_radius - max_displacement + round_off_s1 - 1) / stride_1 + 1 - round_off;
        int ymin = (y_a - 2 * kernel_radius - max_displacement + round_off_s1 - 1) / stride_1 + 1 - round_off;
        int xmax = (x_a - max_displacement + round_off_s1) / stride_1 - round_off;
        int ymax = (y_a - max_displacement + round_off_s1) / stride_1 - round_off;
        ...
        if ((xmax >= 0) && (ymax >= 0) && (xmin <= out_width - 1) && (ymin <= out_height - 1)) {
            ...
            for (int p = -neighborhood_grid_radius; p <= neighborhood_grid_radius; p++) {
                for (int o = -neighborhood_grid_radius; o <= neighborhood_grid_radius; o++) {
                    ...
                    int idx_input_b = ((item * padded_interpolated_height + (y_b + s2p)) * padded_interpolated_width + (x_b + s2o)) * in_channels + k_b;
        ...
        const int input_a_idx = ((y_a - pad_size) * in_width + (x_a - pad_size)) * in_channels + k_a; //
...
//                    float input_a_tmp = input_a[idx_input_a];
                    ...
                    } else if (((s2p%2) == 0) && ((s2o%2) == 1)) {
                        int idx_input_a_01 = ((item * padded_in_height + (y - s2p)) * padded_in_width + (x - s2o)) * in_channels + k;
                        int idx_input_a_02 = ((item * padded_in_height + (y - s2p)) * padded_in_width + (x - s2o + 1)) * in_channels + k;
                        input_a_tmp = (input_a[idx_input_a_01] + input_a[idx_input_a_02]) / 2.0;
                    } else if (((s2p%2) == 1) && ((s2o%2) == 0)) {
                        int idx_input_a_01 = ((item * padded_in_height + (y - s2p)) * padded_in_width + (x - s2o)) * in_channels + k;
                        int idx_input_a_03 = ((item * padded_in_height + (y - s2p + 1)) * padded_in_width + (x - s2o)) * in_channels + k;
                        input_a_tmp = (input_a[idx_input_a_01] + input_a[idx_input_a_03]) / 2.0;
                    } else if (((s2p%2) == 1) && ((s2o%2) == 1)) {
                        int idx_input_a_01 = ((item * padded_in_height + (y - s2p)) * padded_in_width + (x - s2o)) * in_channels + k;
                        int idx_input_a_02 = ((item * padded_in_height + (y - s2p)) * padded_in_width + (x - s2o + 1)) * in_channels + k;
                        int idx_input_a_03 = ((item * padded_in_height + (y - s2p + 1)) * padded_in_width + (x - s2o)) * in_channels + k;
                        int idx_input_a_04 = ((item * padded_in_height + (y - s2p + 1)) * padded_in_width + (x - s2o + 1)) * in_channels + k;
...
CudaLaunchConfig config = GetCudaLaunchConfig(in_count_per_sample_b, device);
