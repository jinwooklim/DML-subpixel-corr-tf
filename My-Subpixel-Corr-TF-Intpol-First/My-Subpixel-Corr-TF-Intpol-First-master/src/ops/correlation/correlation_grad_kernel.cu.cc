...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   int        interpolated_out_height,
   int        interpolated_out_width,
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//                            int idx_gradient = ((item * interpolated_out_height + y) * interpolated_out_width + x) * out_channels + op;
//                            sum += gradient[idx_gradient] * input_b_tmp;
                            int idx_gradient = ((item * interpolated_out_height + y) * interpolated_out_width + x) * out_channels + op;
                            float sample = gradient[idx_gradient];
                            float weight = input_b_tmp / (float)sumelems;
                            if(sample != sample){ // isanan
                                accum_nan += weight;
                                sample = 0.0;
                                weight = 0.0;
                            }
...
//        output_a_gradient[input_a_idx + item * in_count_per_sample_i] = sum / (float)sumelems;
        ...
        if(accum_nan / accum_weight > 0.5){
            output_a_gradient[input_a_idx + item * in_count_per_sample_i] = CUDART_NAN_F;
        }
        else {
            output_a_gradient[input_a_idx + item * in_count_per_sample_i] = sum / accum_weight;
        }
    }
...
//                                    sum += gradient[idx_gradient] * input_a_tmp;
                                    int idx_gradient = ((item * interpolated_out_height + y) * interpolated_out_width + x) * out_channels + op;
                                    float sample = gradient[idx_gradient];
                                    float weight = input_a_tmp / (float)sumelems;
                                    if(sample != sample){ // isanan
                                        accum_nan += weight;
                                        sample = 0.0;
                                        weight = 0.0;
...
//        output_b_gradient[input_b_idx + item * in_count_per_sample_i] = sum;

        if(accum_nan / accum_weight > 0.5){
            output_b_gradient[input_b_idx + item * in_count_per_sample_i] = CUDART_NAN_F;
        }
        else {
            output_b_gradient[input_b_idx + item * in_count_per_sample_i] = sum / accum_weight;
        }
...
//            for (int p = -neighborhood_grid_radius; p <= neighborhood_grid_radius; p++) {
//                for (int o = -neighborhood_grid_radius; o <= neighborhood_grid_radius; o++) {
//                    int s2o = stride_2 * o; ///// int s2o = stride_2 * o;
//                    int s2p = stride_2 * p; ///// int s2p = stride_2 * p;
//
//                    int xmin = (x - 2 * kernel_radius - max_displacement - s2o + round_off_s1 - 1) / stride_1 + 1 - round_off;
//                    int ymin = (y - 2 * kernel_radius - max_displacement - s2p + round_off_s1 - 1) / stride_1 + 1 - round_off;
//                    int xmax = (x - max_displacement - s2o + round_off_s1) / stride_1 - round_off;
...
//            for(int i = -1; i < 1; i++) {
//                for(int j = -1; j < 1; j++){
//                    const int input_b_idx = ((y - pad_size + j) * interpolated_width + (x - pad_size + i)) * in_channels + k;
//                    if(i%2 == 1 && j%2 == 1){
//                        output_b_gradient[input_b_idx + item * in_count_per_sample_i] += (sum / (float)sumelems) / 4.0;
//                    }
//                    else if(i%2 == 1 || j%2 == 1){
//                        output_b_gradient[input_b_idx + item * in_count_per_sample_i] += (sum / (float)sumelems) / 2.0;
//                    }
//                    else if(i%2==0 && j%2==0){
//                        output_b_gradient[input_b_idx + item * in_count_per_sample_i] = (sum / (float)sumelems);
//                    }
...
__global__ void Downsample_corrKernel(
...
//                typename TTypes<float, 4>::Tensor output
                    ...
                    const int out_width,
                    const int channels) {
    const int total_count = batch_size * out_height * out_width * channels;
    ...
    Downsample_corrKernel<<<config.block_count, config.thread_per_block, 0, device.stream()>>>
        (total_count, input, output,
                        in_width, in_height,
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const int        interpolated_out_height,
  const int        interpolated_out_width,
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  const float     *input_b,
  const float     *gradient,
  float           *output_a_gradient) {
CudaLaunchConfig config = GetCudaLaunchConfig(in_count_per_sample_i, device);
  ...
  for (int n = 0; n < batch_size; n++) {
    CorrelateDataBackward0 <<< config.block_count, config.thread_per_block, 0, device.stream() >>> (
      ...
      interpolated_out_height, interpolated_out_width,
      ////
