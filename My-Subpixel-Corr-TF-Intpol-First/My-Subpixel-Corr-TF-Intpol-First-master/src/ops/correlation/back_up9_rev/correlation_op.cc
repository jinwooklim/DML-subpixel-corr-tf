...
Status SetOutput(InferenceContext *c) {
  ...
  // Get the attributes
  // int kernel_size, max_displacement, pad, stride_1, stride_2;
  int kernel_size, max_displacement, pad;
  float stride_1, stride_2, rate;
  ...
  TF_RETURN_IF_ERROR(c->GetAttr("rate", &rate));
  ...
  int64 padded_width  = input_width + 2 * pad;
    int64 interpolated_height = (int)rate * (input_height - 1) + 1;
    int64 interpolated_width = (int)rate * (input_width - 1) + 1;
    int64 padded_interpolated_height = interpolated_height + 2 * pad;
    int64 padded_interpolated_width = interpolated_width + 2 * pad;
...
//  int64 output_width  = (int64)ceil((float)(padded_width - border_size * 2) / (float)stride_1);
  ...
  // int   neighborhood_grid_radius = max_displacement / (int)stride_2;
  int   neighborhood_grid_radius = max_displacement * (int)rate;  
...
//  c->set_output(0, c->MakeShape({ batch, interpolated_height, interpolated_width, output_channels }));
  return Status::OK();
}
