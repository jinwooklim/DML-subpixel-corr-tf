   ...
   const float *gradient)
{
    CUDA_1D_KERNEL_LOOP(index, nthreads) {
        ...
        int k_b_i = index % in_channels;                                     // channels
        int x_b_i = (index / in_channels) % interpolated_width + pad_size;             // w-pos
        int y_b_i = (index / in_channels / interpolated_width) % interpolated_height + pad_size; // h-pos

        // Version # 3
        int k = k_b_i;
        int x = (x_b_i - pad_size)/2 + pad_size;
        int y = (y_b_i - pad_size)/2 + pad_size;
