...
Status SetOutput(InferenceContext *c) {
  ...
  int64 padded_interpolated_height = interpolated_height + ((int)rate * 2 * pad);
  int64 padded_interpolated_width = interpolated_width + ((int)rate * 2 * pad);

  // The size of unreachable border region on each side
  max_displacement = max_displacement * (int)rate;
  int kernel_radius = (kernel_size - 1) / 2;
  int border_size   = max_displacement / (int)rate + kernel_radius;
