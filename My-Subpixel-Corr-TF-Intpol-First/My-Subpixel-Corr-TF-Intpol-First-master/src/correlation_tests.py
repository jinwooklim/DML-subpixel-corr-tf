    ...
    def test_correlationHardCoded(self):
        with tf.device('/gpu:0'):
            with tf.Session('') as sess:
                ...
                input_a = tf.convert_to_tensor(a, tf.float32)
                input_b = tf.convert_to_tensor(b, tf.float32)

                print("a : ", np.shape(a))
                print("b : ", np.shape(b))

                result = correlation(input_a,
                                     input_b,
                                     kernel_size,
                                     max_displacement,
                                     stride_1,
                ...
                grad_a = tf.gradients(result[:, :, :, 0], input_a)
                print(grad_a)
                # print(np.shape(sess.run(grad_a)))

                grad_b = tf.gradients(result[:, :, :, 0], input_b)
                print(grad_b)
                # print(np.shape(sess.run(grad_b)))

                # sess.run([grad_a, grad_b], feed_dict={input_a : a, input_b : b})
                a, b = sess.run([grad_a, grad_b])
