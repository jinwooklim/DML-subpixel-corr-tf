...
class Net(object):
    ...
    def test(self, checkpoint, input_a_path, input_b_path, out_path, save_image=True, save_flo=False):
        ...
        with tf.Session() as sess:
            ...
            if save_image:
                ...
                full_out_path = os.path.join(out_path, unique_name + '_flow.png')
            ...
            pred_flow = predictions['predict_flow2']
            pred_flow = sess.run(pred_flow)[0, :, :, :]

            unique_name = 'flow-' + str(uuid.uuid4())
            if save_image:
                flow_img = flow_to_image(pred_flow)
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow2.png')
                imsave(full_out_path, flow_img)

            if save_flo:
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow2.flo')
                write_flow(pred_flow, full_out_path)

            pred_flow = predictions['predict_flow3']
            pred_flow = sess.run(pred_flow)[0, :, :, :]

            unique_name = 'flow-' + str(uuid.uuid4())
            if save_image:
                flow_img = flow_to_image(pred_flow)
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow3.png')
                imsave(full_out_path, flow_img)

            if save_flo:
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow3.flo')
                write_flow(pred_flow, full_out_path)
            ...
            if save_image:
                flow_img = flow_to_image(pred_flow)
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow4.png')
                imsave(full_out_path, flow_img)

            if save_flo:
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow4.flo')
                write_flow(pred_flow, full_out_path)

            pred_flow = predictions['predict_flow5']
            pred_flow = sess.run(pred_flow)[0, :, :, :]

            unique_name = 'flow-' + str(uuid.uuid4())
            if save_image:
                flow_img = flow_to_image(pred_flow)
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow5.png')
                imsave(full_out_path, flow_img)

            if save_flo:
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow5.flo')
                write_flow(pred_flow, full_out_path)

            pred_flow = predictions['predict_flow6']
            pred_flow = sess.run(pred_flow)[0, :, :, :]

            unique_name = 'flow-' + str(uuid.uuid4())
            if save_image:
                flow_img = flow_to_image(pred_flow)
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow6.png')
                imsave(full_out_path, flow_img)

            if save_flo:
                full_out_path = os.path.join(out_path, unique_name + '_predict_flow6.flo')
                write_flow(pred_flow, full_out_path)
            
    def test_backup(self, checkpoint, input_a_path, input_b_path, out_path, save_image=True, save_flo=False):
        input_a = imread(input_a_path)
        input_b = imread(input_b_path)
    ...
    def train(self, log_dir, training_schedule, input_a, input_b, flow, checkpoints=None):
        ...
        #             for var in variables_to_restore
        #         }
        #         saver = tf.train.Saver(renamed_variables)
        #         with tf.Session() as sess:
        #             saver.save(sess, os.path.join(checkpoint_path, "error"))
        #     exit()
