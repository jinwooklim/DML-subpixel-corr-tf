            ...
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                ...
                with slim.arg_scope([slim.conv2d], weights_regularizer=weights_regularizer):
                    with slim.arg_scope([slim.conv2d], stride=2):
                        ...
                        conv_a_3_resize = tf.image.resize_images(conv_a_3, [(W * 2)-1, (H * 2)-1], method=1) # ResizeMethod.NEAREST_NEIGHBOR
                        conv_b_3_resize = tf.image.resize_images(conv_b_3, [(W * 2)-1, (H * 2)-1], method=0) # ResizeMethod.BILINEAR
                        cc = correlation(conv_a_3_resize, conv_b_3_resize, 1, 40, 2, 1, 40) # train
                        
                        #cc = correlation(conv_a_3, conv_b_3, 1, 20, 1, 2, 20) # original
                    ...
                    #netA_conv = slim.conv2d(conv_a_3, 476, 1, scope='conv_redir')                   
                    # Concatenate along the channels axis
                    net = tf.concat([netA_conv, cc_relu], axis=3)

                    conv3_1 = slim.conv2d(pad(net), 256, 3, scope='conv3_1') # 256
                    #conv3_1 = slim.conv2d(pad(net), 1024, 3, scope='conv3_1') # 256
